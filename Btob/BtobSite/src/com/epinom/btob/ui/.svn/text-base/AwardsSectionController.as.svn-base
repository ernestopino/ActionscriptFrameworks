package com.epinom.btob.ui
{
	...
	public class AwardsSectionController implements ISectionController
	{
		private static var _instance:AwardsSectionController = null;
		private static var _allowInstantiation:Boolean;
		
		public static const SECTION_NAME:String = "awards";
		public static const SECTION_CONTROLLER:String = "awardsSectionController";
		...
		public function AwardsSectionController()
		{
			trace("AwardsSectionController->AwardsSectionController()");
			
			if (!_allowInstantiation) {
				throw new Error("Error: Instantiation failed: Use AwardsSectionController.getInstance() instead of new.");
			} else {				
				_sectionMovie = null;
		...
		public static function getInstance():AwardsSectionController 
		{
			if (_instance == null) 
			{
				_allowInstantiation = true;
				_instance = new AwardsSectionController();
		...
		public function set sectionMovie(value:MovieClip):void {  
			_sectionMovie = value;
			_sectionMovie.addEventListener(TableDataManager.SECTION_EVENT, onAwardsEventHandler, false, 0, true);
		...
		{
			trace("AwardsSectionController->addBackgroundImageContainer()");		
			backgroundImageContainer.name = "awardsBackgroundImageContainer_" + _backgroundImageContainerList.length;
			backgroundImageContainer.visible = true;
			_backgroundImageContainerList.push(backgroundImageContainer);
		}
		
		public function updateAwardsImageInfo():void 
		{
			trace("AwardsSectionController->updateAwardsImageInfo()");
		...
		{
			trace("AwardsSectionController->onBulkSectionLoadedHandler()");
			...
			// Recupero el swf cargado (AWARDS)
			var awards_mc:MovieClip = bulkLoader.getMovieClip(_sectionVO.urlSWF);
			trace(awards_mc);
			...
			this.sectionMovie = awards_mc;
			
			// Actualizando seccion actual
			SiteManager.getInstance().activeSection = awards_mc;
			
			// Agregando SWF cargado al Movieclip que representa la seccion de contenidos					
			SiteManager.getInstance().addInfoToSectionPanel(awards_mc);	
		...
		private function onAwardsEventHandler(evt:EventComplex):void 
		{
			trace("AwardsSectionController->onAwardsEventHandler()");
			trace(evt.data);
