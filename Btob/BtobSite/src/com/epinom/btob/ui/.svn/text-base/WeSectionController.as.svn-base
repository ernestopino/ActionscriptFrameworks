...
{	
	...
	public class WeSectionController implements ISectionController
	{
		private static var _instance:WeSectionController = null;
		private static var _allowInstantiation:Boolean;
		
		public static const SECTION_NAME:String = "we";
		public static const SECTION_CONTROLLER:String = "weSectionController";
		...
		public function WeSectionController()
		{
			if (!_allowInstantiation) {
				throw new Error("Error: Instantiation failed: Use WeSectionController.getInstance() instead of new.");
			} else {				
				_sectionMovie = null;
		...
		public static function getInstance():WeSectionController 
		{
			if (_instance == null) 
			{
				_allowInstantiation = true;
				_instance = new WeSectionController();
		...
		public function set sectionMovie(value:MovieClip):void {  
			_sectionMovie = value;
			_sectionMovie.addEventListener(TableDataManager.SECTION_EVENT, onWeEventHandler, false, 0, true);
		...
		{
			trace("WeSectionController->addBackgroundImageContainer()");		
			backgroundImageContainer.name = "weBackgroundImageContainer_" + _backgroundImageContainerList.length;
			backgroundImageContainer.visible = true;
			_backgroundImageContainerList.push(backgroundImageContainer);
		}
		
		public function updateWeImageInfo():void 
		{
			trace("WeSectionController->updateWeImageInfo()");
		...
		{
			trace("WeSectionController->onBulkSectionLoadedHandler()");
			...
			// Recupero el swf cargado (WE)
			var we_mc:MovieClip = bulkLoader.getMovieClip(_sectionVO.urlSWF);
			trace(we_mc);
			...
			this.sectionMovie = we_mc;
			
			// Actualizando seccion actual
			SiteManager.getInstance().activeSection = we_mc;
			
			// Agregando SWF cargado al Movieclip que representa la seccion de contenidos					
			SiteManager.getInstance().addInfoToSectionPanel(we_mc);	
		...
		private function onWeEventHandler(evt:EventComplex):void 
		{
			trace("WeSectionController->onWeEventHandler()");
			trace(evt.data);
