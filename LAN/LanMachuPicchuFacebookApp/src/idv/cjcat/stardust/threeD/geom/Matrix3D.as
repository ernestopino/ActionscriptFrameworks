package idv.cjcat.stardust.threeD.geom {
	...
	public class Matrix3D {
		...
		public function set(rotationX:Number = 0, rotationY:Number = 0, rotationZ:Number = 0, scaleX:Number = 1, scaleY:Number = 1, scaleZ:Number = 1, tx:Number = 0, ty:Number = 0, tz:Number = 0, rotationOrder:int = 0):Matrix3D {
			identity();
		...
		public function transformThisVec(v:Vec3D):Vec3D {
			v.set((a * v.x + b * v.y + c * v.z + tx), (d * v.x + e * v.y + f * v.z + ty), (g * v.x + h * v.y + i * v.z + tz));
			return v;
		}
		
		public function transform(v:Vec3D):Vec3D {
			return transformThisVec(v.clone());
		}
		
		public function postMultiply(rhs:Matrix3D):Matrix3D {
		...
		public function preMultiply(lhs:Matrix3D):Matrix3D {
			var thisClone:Matrix3D = Matrix3DPool.get(a, b, c, d, e, f, g, h, i, tx, ty, tz);
		...
		public function scale(x:Number, y:Number, z:Number):Matrix3D {
			a *= x; b *= x; c *= x;
			d *= y; e *= y; f *= y;
			g *= z; h *= z; i *= z;
			return this;
		...
		public function translate(x:Number, y:Number, z:Number):Matrix3D {
			tx += x; ty += y; tz += z;
			return this;
		}
